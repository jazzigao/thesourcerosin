...
async function findAvailableMetroPort() {
  const requestedPort = Number.parseInt(
    process.env.EXPO_BUILD_METRO_PORT || '8081',
    10,
  );
  const firstPort = Number.isFinite(requestedPort) ? requestedPort : 8081;
  for (let candidate = firstPort; candidate < firstPort + 50; candidate++) {
    if (await isPortAvailable(candidate)) return candidate;
  }
  throw new Error('No available local port was found for the Expo build.');
}

function getMetroBaseUrl() {
  return `http://localhost:${metroPort}`;
}

async function checkMetroHealth() {
  try {
    const response = await fetch(`${getMetroBaseUrl()}/status`, {
...
async function downloadBundle(platform, timestamp) {
  ...
  const url = new URL(`${getMetroBaseUrl()}/${bundlePath}.bundle`);
...
async function downloadManifest(platform) {
  ...
  try {
    ...
    const response = await fetch(`${getMetroBaseUrl()}/manifest`, {
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      const tempUrl = new URL(`${getMetroBaseUrl()}${originalPath}`);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`${getMetroBaseUrl()}${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get('unstable_path');
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    ...
    bundle = bundle.replace(
      ...
      (_match, capturedPath) => {
        const tempUrl = new URL(`${getMetroBaseUrl()}${capturedPath}`);
        const unstablePath = tempUrl.searchParams.get('unstable_path');
